...
class Engine{
    ...
    fun changeDirection(id: Long, dot: Dot){
        players[id]?.dir = players[id]!!.pos.directionTo(dot)
    }
    ...
    private fun teleportRandom(player: Player){
        player.pos = Dot(maxX * Random.nextDouble(1.0), maxY * Random.nextDouble( 1.0))
    }
    ...
    fun addPlayer(): Player {
        val res = Player()
        teleportRandom(res)
        changeDirection(res.id, Dot(Random.nextDouble(maxX), Random.nextDouble(maxY)))
        players[res.id] = res
        return res
    }
